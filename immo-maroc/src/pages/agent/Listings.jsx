...
const STATUS_CFG = {
  DISPONIBLE: { label: 'Disponible', dot: 'bg-emerald-400', cls: 'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200' },
  VENDU:      { label: 'Vendu',      dot: 'bg-blue-400',    cls: 'bg-blue-50   text-blue-700   ring-1 ring-blue-200'   },
  LOUE:       { label: 'Loué',       dot: 'bg-purple-400',  cls: 'bg-purple-50 text-purple-700 ring-1 ring-purple-200' },
};

const BACKEND_STATUSES = ['DISPONIBLE', 'VENDU', 'LOUE'];

const PURPOSE_CFG = {
  VENTE:    { label: 'Vente',    cls: 'bg-sky-50   text-sky-600'   },
  LOCATION: { label: 'Location', cls: 'bg-amber-50 text-amber-600' },
};
    ...
    <>
      ...
      {open && createPortal(
        <div
          ...
          className="bg-white rounded-2xl shadow-2xl border border-neutral-100 py-2 w-44"
          onMouseDown={e => e.stopPropagation()}
        >
          <p className="px-4 pb-1 text-[10px] font-bold text-neutral-400 uppercase tracking-widest">
            Statut
          </p>
          {BACKEND_STATUSES.map(s => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-2.5 text-xs transition-colors hover:bg-neutral-50 ${active ? 'font-semibold' : 'text-neutral-600'}`}
              >
  ...
  return (
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-5">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: 'Total annonces', value: listings.length,                    icon: Building2,  color: 'text-neutral-700', bg: 'bg-white border border-neutral-200' },
          { label: 'Vues totales',   value: totalViews.toLocaleString('fr-MA'), icon: TrendingUp, color: 'text-primary',     bg: 'bg-primary/5 border border-primary/20' },
          { label: 'En vedette',     value: featuredCount,                       icon: Star,       color: 'text-amber-600',   bg: 'bg-amber-50 border border-amber-200' },
      ...
      {/* ── Filters ── */}
      <div className="bg-white rounded-2xl border border-neutral-100 px-4 py-3 flex flex-col sm:flex-row gap-3 items-start sm:items-center">
        <div className="relative flex-1 w-full">
          ...
          <input
            ...
            style={{ fontSize: '16px' }}
            className="w-full h-9 pl-9 pr-3 rounded-xl border border-neutral-200 bg-neutral-50 text-sm focus:outline-none focus:border-primary focus:bg-white transition-all"
          />
        </div>
      ...
      <div className="bg-white rounded-2xl border border-neutral-100">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <Building2 size={40} className="text-neutral-200 mb-3" />
            <p className="font-semibold text-neutral-500 text-sm">Aucune annonce trouvée</p>
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b border-neutral-100">
                  {['Bien', 'Type', 'Statut', 'Premium', 'Vues', 'Date', 'Actions'].map(h => (
                    <th key={h} className="text-left text-[10px] font-bold text-neutral-400 uppercase tracking-widest px-5 py-3.5 whitespace-nowrap first:pl-6 last:pr-6 bg-neutral-50/70">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {filtered.map((p, idx) => {
                  ...
                  return (
                    <tr key={p.id} className={`group hover:bg-neutral-50/60 transition-colors ${!isLast ? 'border-b border-neutral-100' : ''}`}>
                      ...
                      <td className="px-5 py-4 pl-6 min-w-[240px]">
                        <div className="flex items-center gap-3">
                          <div className="relative shrink-0">
                            ...
                            />
                            {p.featured && (
                              <span className="absolute -top-1 -right-1 w-5 h-5 bg-amber-400 rounded-full flex items-center justify-center ring-2 ring-white shadow-sm">
                                <Star size={9} className="text-white fill-white" />
                              </span>
                            )}
                          </div>
                          <div className="min-w-0">
                            <p className="font-semibold text-neutral-900 text-sm truncate max-w-[200px]">{p.title}</p>
                            <p className="text-xs text-neutral-400 mt-0.5 truncate max-w-[200px]">{infoLine(p) || '—'}</p>
                      ...
                      <td className="px-5 py-4 whitespace-nowrap">
                        <span className="text-xs text-neutral-600 bg-neutral-100 px-2.5 py-1 rounded-lg font-medium">
                        ...
                        <button
                          ...
                          className={`group/star inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-full text-xs font-semibold transition-all ${
                            p.featured
                              ...
                              : 'bg-neutral-100 text-neutral-400 hover:bg-amber-50 hover:text-amber-500 hover:ring-1 hover:ring-amber-200'
                          }`}
                        >
                          <Star
                            size={12}
                      ...
                      <td className="px-5 py-4 pr-6">
                        <div className="flex items-center gap-0.5 opacity-60 group-hover:opacity-100 transition-opacity">
                          <Link to={`/propriete/${p.id}`} className="p-2 rounded-lg text-neutral-500 hover:text-primary hover:bg-primary/10 transition-colors" title="Voir">
                            <Eye size={15} />
                          </Link>
                          <Link to={`/agent/modifier/${p.id}`} className="p-2 rounded-lg text-neutral-500 hover:text-emerald-600 hover:bg-emerald-50 transition-colors" title="Modifier">
                            <Pencil size={15} />
                          </Link>
                        </div>
                      </td>
