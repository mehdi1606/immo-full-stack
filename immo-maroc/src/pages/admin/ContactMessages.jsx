...
export default function ContactMessages() {
  ...
  return (
    <div className="p-3 sm:p-6 max-w-7xl mx-auto space-y-6">
      <div>
        <h1 className="font-serif text-2xl font-bold text-neutral-900">Messages de contact</h1>
        <p className="text-neutral-500 text-sm">{messages.length} message{messages.length !== 1 ? 's' : ''} reçu{messages.length !== 1 ? 's' : ''}</p>
      </div>

      {/* Summary cards */}
      <div className="grid grid-cols-3 gap-3">
        {[
          { key: 'NOUVEAU', color: 'bg-blue-50 text-blue-700'       },
          { key: 'LU',      color: 'bg-amber-50 text-amber-700'     },
          { key: 'REPONDU', color: 'bg-emerald-50 text-emerald-700' },
        ].map(({ key, color }) => (
          <button key={key}
      ...
      <div className="bg-white rounded-2xl shadow-card p-4 flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input type="text" className="form-input pl-9 py-2 text-sm" placeholder="Nom, email, sujet…"
            value={q} onChange={e => setQ(e.target.value)} />
        </div>
        <select className="form-select py-2 text-sm w-auto" value={filterStatus} onChange={e => setFilterStatus(e.target.value)}>
        ...
        {(q || filterStatus) && (
          <button onClick={() => { setQ(''); setFilterStatus(''); }}
            className="text-xs text-neutral-500 hover:text-red-500 transition-colors px-2">
            Réinitialiser
          </button>
        )}
      </div>

      {error && <div className="bg-red-50 text-red-600 rounded-xl px-5 py-4 text-sm">{error}</div>}
      ...
      <div className={`grid gap-6 ${selected ? 'grid-cols-1 md:grid-cols-2 lg:grid-cols-5' : 'grid-cols-1'}`}>
                ...
                {filtered.map(msg => {
                  ...
                  return (
                    <div key={msg.id}
                      ...
                      className={`p-4 cursor-pointer hover:bg-neutral-50 transition-colors ${selected?.id === msg.id ? 'bg-primary/5 border-l-4 border-primary' : ''} ${msg.status === 'NOUVEAU' ? 'font-semibold' : ''}`}>
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            <p className="text-neutral-900 text-sm truncate">{msg.name}</p>
                            <p className="text-neutral-400 text-xs truncate">{msg.subject}</p>
                          </div>
                        </div>
                        <div className="flex flex-col items-end gap-1 shrink-0">
                          ...
                          <span className="text-neutral-400 text-[10px]">{fmt(msg.createdAt)}</span>
                        </div>
                      </div>
                      <p className="text-neutral-400 text-xs mt-2 line-clamp-1">{msg.message}</p>
                    </div>
                  );
                })}
              </div>
        ...
        {selected && (
          <div className="lg:col-span-3">
            <div className="bg-white rounded-2xl shadow-card p-6 sticky top-6 space-y-5">
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="font-serif text-xl font-bold text-neutral-900">{selected.name}</h3>
                  <p className="text-neutral-400 text-sm">{fmt(selected.createdAt)}</p>
                </div>
                <div className="flex items-center gap-2">
                  <span className={`text-xs px-3 py-1 rounded-full font-medium ${STATUS_STYLE[selected.status]}`}>
                    {STATUS_LABEL[selected.status]}
                  </span>
                  <button onClick={() => setSelected(null)} className="text-neutral-400 hover:text-neutral-600 transition-colors">
                    <X size={18} />
                  </button>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                {[
                  ...
                  <div key={label} className="bg-neutral-50 rounded-xl p-3">
                    ...
                    <p className="font-medium text-neutral-900 text-sm">{value}</p>
                  </div>
                ))}
              </div>

              <div>
                <p className="text-xs text-neutral-400 mb-2 font-medium uppercase tracking-wide">Message</p>
              ...
              <div className="flex flex-wrap gap-3 pt-2 border-t border-neutral-100">
                ...
                {selected.phone && (
                  <a href={`tel:${selected.phone}`}
                    className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-neutral-200 hover:border-primary hover:text-primary transition-colors text-sm">
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                <p className="text-xs text-neutral-400 w-full">Changer le statut :</p>
